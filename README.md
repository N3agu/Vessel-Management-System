...
4.  **Setup the Database:** Choose **one** of the following methods:

    **Method A: Using EF Core Migrations (Recommended if you have .NET SDK & EF Tools)**
    ...
    **Method B: Using the Provided SQL Script**
    * Locate the SQL script at `/Database/database.sql` in the repository root.
    * Open the script using a SQL Server management tool (like SQL Server Management Studio (SSMS) or Azure Data Studio).
    * Connect to your target SQL Server instance.
    * **Important:** Ensure you are targeting the correct database name specified in your connection string (`VesselManagementDB`). You might need to create the database first (`CREATE DATABASE VesselManagementDB_Test; USE VesselManagementDB;`) or modify the script.
    * Execute the entire script. This will create the necessary tables, relationships, indexes, and insert initial seed data.
5.  **Run the API:**
